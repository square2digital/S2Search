...
# build everything - (do not delete images) (Delete S2 Namespace) - exclude elastic, kibana and  redundant includeConfigAPI & includeCRAPI
# cls; cd "E:\devops\K8s.Local.Cluster.Setup\local"; .\deployment-script.ps1 -includeElasticUI $false -includeSearchUI $true -includeAdminUI $true -includeConfigAPI $false -includeSearchAPI $true -includeElasticAPI $false -includeCRAPI $false -includeRedis $true -includeSftpGo $true -includeElastic $false -deleteAllImages $false -includeAdminAPI $true -deleteS2Namespace $true
...
|  _  ||_  /| | | || '__| / _ \ |  _  ||    \  `--. \
| | | | / / | |_| || |   |  __/ | | | || |\  \/\__/ /
\_| |_//___| \__,_||_|    \___| \_| |_/\_| \_/\____/                                                                                                        
...
if ($deleteS2Namespace) {
    ...
    # delete S2 Namespace
    Write-Color -Text "Before $S2Namespace & $IngressNginxNamespace Delete" -Color Yellow
    kubectl get all -n $S2Namespace
    kubectl get all -n $IngressNginxNamespace
    ...
    kubectl delete all --all -n $IngressNginxNamespace
    ...
    Write-Color -Text "After $S2Namespace & $IngressNginxNamespace Delete" -Color Yellow
    kubectl get all -n $S2Namespace
    kubectl get all -n $IngressNginxNamespace
}
