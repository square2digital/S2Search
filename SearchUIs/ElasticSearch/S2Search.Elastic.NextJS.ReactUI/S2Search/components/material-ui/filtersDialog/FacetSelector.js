  ...
  const buildFacetSelectors = (isChecked) => {
    ...
    if (props.facet.type === "range") {
      selectedFacetData.luceneExpression = `${
        props.selectedFacet
      }):>${FormatStringOrNumeric(props.facet.from)} AND (${
        props.selectedFacet
      }):<${FormatStringOrNumeric(props.facet.to)}`;
    } else {
      if (FacetToParseAsNumeric.includes(selectedFacetData.facetKey)) {
        selectedFacetData.luceneExpression = `${
          props.selectedFacet
        }:${FormatStringOrNumeric(props.facet.value)}`;
      } else {
        selectedFacetData.luceneExpression = `${props.selectedFacet}:${props.facet.value}`;
      }
    }
