...
using S2Search.Backend.Services.AzureFunctions.FeedServices.Extensions;
    ...
    public class AzureSearchManager : IAzureSearchManager
    {
        ...
        private async Task EnableSynoynmsSafetly(SearchIndexClient indexClient, SearchIndex searchIndex)
        {
            for (int i = 0; i < _enableSynonymsMaxTries; ++i)
            {
                ...
                {
                    _logger.LogWarning(requestFailedEx, $"Warning {nameof(EnableSynoynmsSafetly)} failed | Attempt({i}/{_enableSynonymsMaxTries}). | Message: {requestFailedEx.Message}");
                }
                catch (Exception ex)
                {
